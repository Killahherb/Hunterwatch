...
local PROCS = {
    ...
    {
        label = "Searing",
        playerOnly = true,  -- only check player auras (ignore target debuffs)
...
-- helper: search for aura matching any pattern
-- if playerOnly is true, only check player buffs/debuffs (not target)
local function FindAura(patterns, playerOnly)
    local sources = {
        { unit = "player", func = UnitBuff },
        { unit = "player", func = UnitDebuff },
    }
    if not playerOnly and UnitExists("target") then
...
local function Scan()
    for idx, proc in ipairs(PROCS) do
        local found = FindAura(proc.match, proc.playerOnly)
...
DEFAULT_CHAT_FRAME:AddMessage("|cff00ccffHunterWatch v1.6|r loaded. Click minimap icon or type /hw")
