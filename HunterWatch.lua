...
local PROCS = {
    ...
    {
        label = "Searing",
        -- TurtleWoW Searing = Multi-Shot proc; try all possible texture keywords
    ...
    {
        label = "BlkArrow",
        -- TurtleWoW Black Arrow = Arcane Shot proc; try all possible texture keywords
...
local function Scan()
    for idx, proc in ipairs(PROCS) do
        ...
        -- Build lowercase match list
        local patterns = proc.match
        if type(patterns) == "string" then patterns = { patterns } end

        -- Check all sources: player buffs, player debuffs, target debuffs/buffs
        local sources = {
        ...
        for _, src in ipairs(sources) do
            ...
            while true do
                ...
                local bLower = strlower(b)
                for _, pat in ipairs(patterns) do
                    if b == pat or strfind(bLower, strlower(pat)) then
                        found = true; break
                    end
                end
                if found then break end
...
DEFAULT_CHAT_FRAME:AddMessage("|cff00ccffHunterWatch v1.4|r loaded. Click minimap icon or type /hw")
