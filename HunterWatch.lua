...
local function Scan()
    for idx, proc in ipairs(PROCS) do
        local found = false
        local matchLower = strlower(proc.match)

        -- Check all sources: player buffs, player debuffs, target debuffs
        -- TurtleWoW custom abilities may appear in unexpected places
        local sources = {
        ...
        for _, src in ipairs(sources) do
            ...
            while true do
                ...
                if b == proc.match then
                    found = true; break
                elseif strfind(strlower(b), matchLower) then
                    found = true; break
                end
                i = i + 1
            end
...
DEFAULT_CHAT_FRAME:AddMessage("|cff00ccffHunterWatch v1.3|r loaded. Click minimap icon or type /hw")
