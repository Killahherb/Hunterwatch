...
local PROCS = {
    {
        label = "LnL",
        match = { LNL_ICON, "lockandload" },
        icon  = LNL_ICON,
        glow  = {1.0, 0.85, 0.0},
    },
    {
        label = "Searing",
        requireLnL = true,  -- only highlight when LnL is also active
        match = { "spell_fire", "searing", "upgrademoonglaive", "multi" },
    ...
    },
    {
        label = "BlkArrow",
        requireLnL = true,
        match = { "darkarrow", "blackarrow", "impalingbolt", "arcane" },
    ...
    },
    {
        label = "Poison",
        requireLnL = true,
        match = { "poison", "nature_slow", "serpentsting", "sting" },
...
-- helper: check if a texture path matches any pattern in a list
local function MatchesAny(texture, patterns)
    if type(patterns) == "string" then patterns = { patterns } end
    local tLower = strlower(texture)
    for _, pat in ipairs(patterns) do
        if texture == pat or strfind(tLower, strlower(pat)) then
            return true
        end
    end
    return false
...
-- helper: search player buffs/debuffs + target buffs/debuffs for a pattern list
local function FindAura(patterns)
    local sources = {
        { unit = "player", func = UnitBuff },
        { unit = "player", func = UnitDebuff },
    }
    if UnitExists("target") then
        sources[3] = { unit = "target", func = UnitDebuff }
        sources[4] = { unit = "target", func = UnitBuff }
    end

    for _, src in ipairs(sources) do
        local i = 1
        while true do
            local b = src.func(src.unit, i)
            if not b then break end
            if MatchesAny(b, patterns) then return true end
            i = i + 1
        end
    end
    return false
...
local function Scan()
    -- First pass: detect LnL (always index 1)
    local lnlActive = FindAura(PROCS[1].match)
    ...
    for idx, proc in ipairs(PROCS) do
        ...
        if idx == 1 then
            ...
            -- Only highlight if LnL is also active (prevents permanent
            -- glow from normal Serpent Sting / Arcane Shot debuffs)
            if proc.requireLnL and not lnlActive then
                found = false
            else
                found = FindAura(proc.match)
            end
        end
...
MakeButton(gui, -250, "Debug: Dump Buff/Debuff Textures", 220,
    function()
        ...
        DEFAULT_CHAT_FRAME:AddMessage("|cff00ccffHunterWatch DEBUG:|r --- Player Debuffs ---")
        i = 1
        while true do
        ...
        DEFAULT_CHAT_FRAME:AddMessage("|cff00ccffHunterWatch DEBUG:|r --- Target Buffs ---")
        if UnitExists("target") then
            i = 1
            while true do
...
DEFAULT_CHAT_FRAME:AddMessage("|cff00ccffHunterWatch v1.5|r loaded. Click minimap icon or type /hw")
